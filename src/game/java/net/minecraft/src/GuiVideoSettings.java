...
public class GuiVideoSettings extends GuiScreen {
	...
	private static EnumOptions[] videoOptions = new EnumOptions[] { EnumOptions.GRAPHICS, EnumOptions.RENDER_DISTANCE,
			EnumOptions.AMBIENT_OCCLUSION, EnumOptions.ANAGLYPH, EnumOptions.VIEW_BOBBING, EnumOptions.GUI_SCALE,
			EnumOptions.VSYNC, EnumOptions.FANCY_GRASS, EnumOptions.GAMMA };
		...
		for (int var5 = 0; var5 < var4; ++var5) {
			...
			if (!var6.getEnumFloat()) {
				b = new GuiSmallButton(var6.returnEnumOrdinal(), this.width / 2 - 155 + var2 % 2 * 160,
						this.height / 6 + 24 * (var2 >> 1), var6, this.guiGameSettings.getKeyBinding(var6));
			} else {
				b = new GuiSlider(var6.returnEnumOrdinal(), this.width / 2 - 155 + var2 % 2 * 160,
						this.height / 6 + 24 * (var2 >> 1), var6, this.guiGameSettings.getKeyBinding(var6),
						this.guiGameSettings.getOptionFloatValue(var6));
			}

			this.controlList.add(b);

			// if ((var5 == 6 && EagRuntime.getPlatformType() == EnumPlatformType.WASM_GC))
			// b.enabled = false;
