...
public class EntityLightningBolt extends EntityWeatherEffect {
	...
	public EntityLightningBolt(World var1, double var2, double var4, double var6) {
		...
		if (var1.difficultySetting >= 2 && var1.doChunksNearChunkExist(MathHelper.floor_double(var2),
				MathHelper.floor_double(var4), MathHelper.floor_double(var6), 10)) {
	...
	public void onUpdate() {
		...
		if (this.lightningState < 0) {
			if (this.boltLivingTime == 0) {
				...
				this.boltVertex = this.rand.nextLong();
				if (this.worldObj.doChunksNearChunkExist(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY),
		...
		if (this.lightningState >= 0) {
			...
			List var7 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this,
					AxisAlignedBB.getBoundingBoxFromPool(this.posX - var6, this.posY - var6, this.posZ - var6, this.posX + var6,
