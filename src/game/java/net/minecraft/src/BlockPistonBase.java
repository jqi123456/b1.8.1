...
public class BlockPistonBase extends Block {
	...
	public int getBlockTextureFromSideAndMetadata(int var1, int var2) {
		...
		return var3 > 5 ? this.blockIndexInTexture
				: (var1 == var3
						? (!isExtended(var2) && this.minX <= 0.0D && this.minY <= 0.0D && this.minZ <= 0.0D && this.maxX >= 1.0D
								&& this.maxY >= 1.0D && this.maxZ >= 1.0D ? this.blockIndexInTexture : 110)
						: (var1 == PistonBlockTextures.field_31057_a[var3] ? 109 : 108));
	}
	...
	private boolean isIndirectlyPowered(World var1, int var2, int var3, int var4, int var5) {
		return var5 != 0 && var1.isBlockIndirectlyProvidingPowerTo(var2, var3 - 1, var4, 0) ? true
				: (var5 != 1 && var1.isBlockIndirectlyProvidingPowerTo(var2, var3 + 1, var4, 1) ? true
						: (var5 != 2 && var1.isBlockIndirectlyProvidingPowerTo(var2, var3, var4 - 1, 2) ? true
								: (var5 != 3 && var1.isBlockIndirectlyProvidingPowerTo(var2, var3, var4 + 1, 3) ? true
										: (var5 != 5 && var1.isBlockIndirectlyProvidingPowerTo(var2 + 1, var3, var4, 5) ? true
												: (var5 != 4 && var1.isBlockIndirectlyProvidingPowerTo(var2 - 1, var3, var4, 4) ? true
														: (var1.isBlockIndirectlyProvidingPowerTo(var2, var3, var4, 0) ? true
																: (var1.isBlockIndirectlyProvidingPowerTo(var2, var3 + 2, var4, 1) ? true
																		: (var1.isBlockIndirectlyProvidingPowerTo(var2, var3 + 1, var4 - 1, 2) ? true
																				: (var1.isBlockIndirectlyProvidingPowerTo(var2, var3 + 1, var4 + 1, 3) ? true
																						: (var1.isBlockIndirectlyProvidingPowerTo(var2 - 1, var3 + 1, var4, 4)
																								? true
																								: var1.isBlockIndirectlyProvidingPowerTo(var2 + 1, var3 + 1, var4,
																										5)))))))))));
	}
	...
	public void playBlock(World var1, int var2, int var3, int var4, int var5, int var6) {
		...
		if (var5 == 0) {
			if (this.tryExtend(var1, var2, var3, var4, var6)) {
				...
				var1.playSoundEffect((double) var2 + 0.5D, (double) var3 + 0.5D, (double) var4 + 0.5D, "tile.piston.out", 0.5F,
						var1.rand.nextFloat() * 0.25F + 0.6F);
			}
		} else if (var5 == 1) {
			TileEntity var8 = var1.getBlockTileEntity(var2 + PistonBlockTextures.offsetsXForSide[var6],
					var3 + PistonBlockTextures.offsetsYForSide[var6], var4 + PistonBlockTextures.offsetsZForSide[var6]);
			...
			var1.setBlockTileEntity(var2, var3, var4,
					BlockPistonMoving.getNewTileEntity(this.blockID, var6, var6, false, true));
			if (this.isSticky) {
				...
				if (var14 || var12 <= 0 || !canPushBlock(var12, var1, var9, var10, var11, false)
						|| Block.blocksList[var12].getMobilityFlag() != 0 && var12 != Block.pistonBase.blockID
								&& var12 != Block.pistonStickyBase.blockID) {
					if (!var14) {
						this.ignoreUpdates = false;
						var1.setBlockWithNotify(var2 + PistonBlockTextures.offsetsXForSide[var6],
								var3 + PistonBlockTextures.offsetsYForSide[var6], var4 + PistonBlockTextures.offsetsZForSide[var6], 0);
						this.ignoreUpdates = true;
					}
				} else {
					this.ignoreUpdates = false;
					...
					var1.setBlockTileEntity(var2, var3, var4,
							BlockPistonMoving.getNewTileEntity(var12, var13, var6, false, false));
				}
			} else {
				this.ignoreUpdates = false;
				var1.setBlockWithNotify(var2 + PistonBlockTextures.offsetsXForSide[var6],
			...
			var1.playSoundEffect((double) var2 + 0.5D, (double) var3 + 0.5D, (double) var4 + 0.5D, "tile.piston.in", 0.5F,
					var1.rand.nextFloat() * 0.15F + 0.6F);
		}
	...
	private static int determineOrientation(World var0, int var1, int var2, int var3, EntityPlayer var4) {
		if (MathHelper.abs((float) var4.posX - (float) var1) < 2.0F
				&& MathHelper.abs((float) var4.posZ - (float) var3) < 2.0F) {
		...
		while (true) {
			...
			while (var6 != var2 || var7 != var3 || var8 != var4) {
				...
				if (var12 == this.blockID && var9 == var2 && var10 == var3 && var11 == var4) {
					...
					var1.setBlockTileEntity(var6, var7, var8, BlockPistonMoving.getNewTileEntity(Block.pistonExtension.blockID,
							var5 | (this.isSticky ? 8 : 0), var5, true, false));
				} else {
					var1.setBlockAndMetadata(var6, var7, var8, Block.pistonMoving.blockID, var13);
					var1.setBlockTileEntity(var6, var7, var8,
							BlockPistonMoving.getNewTileEntity(var12, var13, var5, true, false));
				}
