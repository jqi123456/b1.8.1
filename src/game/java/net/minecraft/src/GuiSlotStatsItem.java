		...
		while (var2.hasNext()) {
			...
			if (GuiStats.getStatsFileWriter(var1).writeStat(var3) > 0) {
				var4 = true;
					...
					&& GuiStats.getStatsFileWriter(var1).writeStat(StatList.objectBreakStats[var5]) > 0) {
				var4 = true;
			} else if (StatList.objectCraftStats[var5] != null
					&& GuiStats.getStatsFileWriter(var1).writeStat(StatList.objectCraftStats[var5]) > 0) {
				var4 = true;
			}
